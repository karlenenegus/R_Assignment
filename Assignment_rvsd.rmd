...
```{r}
print("geno")
print(paste(nrow(geno), "r x ", ncol(geno), "c", sep=""))
print(typeof(geno))
class(geno)
print("")
print("POS")
print(paste(nrow(POS), "r x ", ncol(POS), "c", sep=""))
print(typeof(POS))
class(POS)
```
## Data Processing
...
```{r}
for (species in c("maize", "teosinte")){ #replicating for each species
  
  if (species=="maize"){
    sp.group <- c("ZMMIL|ZMMLR|ZMMMR") #assign terms for species
  } else if (species=="teosinte"){
    sp.group <- c("ZMPBA|ZMPIL|ZMPJA") #assign terms for species
  }
  
  subset.geno <- t(geno[grep(sp.group, geno$Group),c(-2,-3)]) #subset genos by species terms
  colnames(subset.geno) <- subset.geno[1,] #column names
  subset.geno <- subset.geno[2:nrow(subset.geno),] #column names
  
  if(all(rownames(subset.geno)==POS$SNP_ID)){ #merging POS & subset genos if SNP IDs match
    geno.POS.merge <- cbind(POS[,c(1,3,4)], subset.geno) 
    write.table(geno.POS.merge, file = paste(species, "_genotype_positions.txt", sep = ""), sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE) #write a merged table to file
    } else{
    print("ERROR: SNP IDs do not match") 
    remove(geno.POS.merge)
    }
  
  all.chr <- unique(geno.POS.merge$Chromosome) #store list of chromosome names
  nonnumeric.chr <- all.chr[grep("[a-z]", all.chr, ignore.case = TRUE)] #list which names are nonnumeric
  numeric.chr <- all.chr[grep("[0-9]", all.chr)] #list which names are numeric
  numeric.positions <- unique(geno.POS.merge$Position)[grep("[a-z]", unique(geno.POS.merge$Position), ignore.case = TRUE, invert = TRUE)] #list positions which are numeric
  
  for(chrm in all.chr){ #repeat for all chromosome names
    if(chrm %in% intersect(nonnumeric.chr, numeric.chr)){
      print("Non-standard Chromosome Name: Numeric and Non-numeric")
    } else if(chrm %in% numeric.chr){ #handle numeric chromosomes differently than nonnumeric
      geno.POS.merge %>%
        filter(Position %in% numeric.positions) %>% #Removes positions which are not numbers
        filter(Chromosome==chrm) %>% #filters data by current chromosome
        arrange(as.numeric(Position)) %T>% #sorts data by position
        write.table(file = paste(species, "/", species, "_chr_", chrm, "_asc.txt", sep = ""), col.names = TRUE, row.names = FALSE, quote = FALSE) %>% #write ascending order file for chromosome
        arrange(desc(as.numeric(Position))) %>% #sort descending
        lapply(., gsub, pattern = "\\?", replacement = "\\-") %>% do.call(cbind, .) %>% # Replaces missing data symbol ?/? with -/-
        write.table(file = paste(species, "/", species, "_chr_", chrm, "_desc.txt", sep = ""), sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE) #write descending order file for chromosome
    } else if (chrm %in% nonnumeric.chr){ #instructions for nonnumeric chromosomes
      geno.POS.merge %>%
        filter(Chromosome==chrm | Position==chrm) %>% #filter for chr name or matching position designation
        arrange(Position) %>%
        write.table(file = paste(species, "/", species, "_chr_", chrm, ".txt", sep = ""), sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE) #write to file for chromosome name
    } else {
...
### Plot 1 - SNPs on each chromosome
...
### Plot 2 - SNPs by chromosome position 
...
### Plot 6 - Genotype Group Proportions
...
### Plot 7 - Proportions by Sample ID
...
### Plot 8 - SNPs with heterozygosity proportion differences between Maize and Teosinte
